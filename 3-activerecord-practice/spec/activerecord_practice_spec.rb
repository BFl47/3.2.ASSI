...
describe 'ActiveRecord practice' do
  ...
  describe 'to find' do
    ...
    describe 'using .where:' do
      ...
      specify 'with last names starting with "B", sorted by birthdate (hint: lookup ActiveRecord `order`)' do
        expect(Customer.last_names_starting_with_b.map(&:id)).to eq( [25,23,4,28,18,21,29,1] )
      end
    end
    describe 'without needing .where' do
      specify '20 youngest customers, in any order (hint: lookup ActiveRecord `limit`)' do
  ...
  describe 'to update' do
    ...
    specify 'the birthdate of Gussie Murray to February 8,2004 (HINT: lookup `DateTime.parse`)' do
